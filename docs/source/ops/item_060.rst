...
To enable the use of the direct PSO pulses to trigger the detector or the the new interlaced sequence generated by the write_PSO_array()routine we configured a multiplexer(MUX2-1) setting input0 = PSO and input1 trifILF. In this was we can set the multiplex select PV to 0 or 1 to choose what we send to the camera trigger.
